...
/**
 * Principal cuando se agregaun nuevo computador
 */
...
export default function FrmOrdeninterna(props){ //Aqui declaras el componente funcion
...
const history = useHistory();
    ...
    const traerRegionales = async () => {
        try{
        await db.collection('Regionales').onSnapshot((objeReg) => { //Para que es el onSnapshot
            const array = [];
            objeReg.forEach((obj) => {
                const auxobj = {
                    ...obj.data(),
                    nombre: obj.data().nombre,
                    compania: obj.data().compania,
                    id:obj.id
                }
                array.push(auxobj);
                    ...
                    >
                        {
                            listReg.map((obj) => {
                                ...
                                if (obj.nombre != 'undefined') {
                                    return <MenuItem onClick={(ev) => llenarespacio(obj.id)} key={obj.id} value={obj.nombre}>{obj.nombre+"-"+obj.compania}</MenuItem>
                                }
                            })
                        }
                    </Select>
